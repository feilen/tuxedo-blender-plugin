#if !BAKEFIXER_CUBEHIPS
#define BAKEFIXER_CUBEHIPS
#if UNITY_EDITOR
using System.Text;
...
[InitializeOnLoad]
public class BakeFixer_CubeHips : ScriptableObject
{
    static BakeFixer_CubeHips()
    {
        EditorApplication.hierarchyChanged += Update;
        Debug.Log("BakeFixer loaded for CubeHips");
    }
    ...
    static void Update()
    {
        ...
        {
            // For each armature on the scene who owns "Armature", "Static" and "CubeHips",
            // get their SkinnedMeshRenderer components and edit the anchor point
            if (obj.transform.Find("Armature") != null &&
                    obj.transform.Find("CubeHips") != null &&
                    obj.transform.Find("Static") != null)
            {
                var body = obj.transform.Find("CubeHips");
